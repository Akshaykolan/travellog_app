...
class _EntryDetailScreenState extends ConsumerState<EntryDetailScreen> {
  ...
  void initState() {
    ...
    if (allPhotos.isNotEmpty) {
      _timer = Timer.periodic(const Duration(seconds: 3), (timer) {
        if (_pageController.hasClients && allPhotos.length > 1) {
  ...
  Future<void> _confirmAndDeleteEntry() async {
    ...
    if (confirm == true) {
      ref.read(entriesProvider.notifier).deleteEntry(widget.entryId);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final entry = entries.firstWhere((e) => e.id == widget.entryId);
    // Find the index of the entry for the edit screen
    final entryIndex = entries.indexWhere((e) => e.id == widget.entryId);

    final allPhotos = entry.photos;
    final tags = entry.tags;
    final DateTime dateTime = DateTime.parse(entry.dateTime);
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) =>
                      // Pass the entry and the found index
                      AddEntryPage(entry: entry, index: entryIndex),
                ),
              );
            },
          ),
